...
class GaussianDiffusion(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        if not hparams["fix_cond_net_parms"]:
            # print("cond_net_out:", cond_net_out.shape)
            # print("labels:", labels.shape)

            # cond_net_out: torch.Size([2, 13, 64, 64])
            # labels: torch.Size([2, 64, 64])

            ret["cls_loss"] = self.criterion_sat(cond_net_out, labels)
    ...
    def p_losses(
        ...
        for tps in range(img_lr_up.shape[1]):
            ...
            # cond 0: torch.Size([2, 12, 64, 64, 64])
            # cond 1: torch.Size([2, 12, 128, 32, 32])
            # cond 2: torch.Size([2, 12, 256, 16, 16])
    ...
    def sample(
        ...
        if hparams["use_highresnet_ltae"] or hparams["use_rrdb_ltae"]:
            cond_net_out, _, _, cond = self.cond_net(img_lr, dates)
        else:
            cond_net_out = img_lr_up
            cond = [img_lr]
