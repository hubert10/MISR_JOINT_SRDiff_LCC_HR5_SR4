...
class UNetDecoder(nn.Module):
    ...
    def forward(self, x, h, w):
        res0, res1, res2, res3 = x[0], x[1], x[2], x[-1]
        dec_features = []
        x = self.b3(self.pre_conv(res3))
        ...
        x = self.p1(x, res1)
        x = self.b1(x)
        dec_features.append(x)  # save after b1

        x = self.p0(x, res0)
        dec_features.append(x)  # save after p0

        # target resolution = resolution of last feature
        target_size = dec_features[-1].shape[2:]  # (64, 64)

        # upsample all features to the same full spatial resolution
        upsampled_features = [
